...
filename = 'voigt_'+str(step_num)+'.h5'

# film density
rholist = np.linspace(500, 2000, step_num).astype(float)
# film thickness
hlist = np.linspace(0.01e-6, 2e-6, step_num).astype(float)
# shear modulus
mulist = np.logspace(6, 12, step_num).astype(float)
# viscosity
etalist = np.logspace(-12, -2, step_num).astype(float)
...
for rho0 in rholist:
    for h0 in hlist:
        ...
        h5store(filename, 's'+str(df_num).zfill(9), df0, **{'rho': rho0, 'h': h0})

        if df_num % 100 == 0:
            print('generating sample %i / %i' %(row, tot_points))
            			
            runtime = time.time() - start_time
            print('elapsed runtime = %i sec (%0.2f min)' %(
