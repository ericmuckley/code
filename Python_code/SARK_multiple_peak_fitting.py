...
data_skip = 6 #skip first couple points
...
pressures = np.array([os.path.splitext(os.path.basename(
        x))[0] for x in folder]).astype(float)
...
for i in range(len(folder)):
    ...
    spec = 1e3*np.array(data_raw['Gp'])[data_skip:]
    ...
    peaks_detected0,_ = detect_peaks(spec, .002, freq)
    ...
    guess = [peaks_detected[0,0], peaks_detected[0,1]*2, .02,#1*(i+1)*0.001, 
             peaks_detected[1,0], peaks_detected[1,1]*2, .02,#1*(i+1)*0.001,
             peaks_detected[2,0], peaks_detected[2,1]*2, .02]#1*(i+1)*0.001]
