...
#%% USER INPUTS
data_folder = glob.glob('C:\\Users\\a6q\\exp_data\\2019-01-28_PSS_CV/*')
data_folder.sort(key=os.path.getmtime)

rh_file = 'C:\\Users\\a6q\\exp_data\\2019-01-28_rh'

time_table = get_time_table(rh_file, pressure_col_name='rh_setpoint')

use_all_files = False
if not use_all_files:
    good_files = get_good_files(time_table, data_folder)
    time_table['files'] = good_files
if use_all_files:
...
sweep_rates = ['200 mV/s', '50 mV/s', '25 mV/s']
...
file_pressures = match_files_to_pressures(time_table, data_folder)
...
for i, file in enumerate(good_files):
    ...
    for j, col in enumerate(list(data0)[1:]):
        current = np.array(data0[col])*1e9
        current -= current[0]
    ...
    config_plot('Bias (V)', 'Current (nA)',
               setlimits=True, limits=[-0.55, 0.55, -150, 150])
    plt.title('%i%% RH' %file_pressures[i], fontsize=12)
...
for i in range(len(sweep_rates)):
    plt.plot(time_table['pressure'], dic['areas'][:,i], lw=0.5,
             marker='o', markersize=1.5, label=sweep_rates[i])  
config_plot('Time', 'Area (uA V)')
plt.legend(fontsize=12)
plt.show()
