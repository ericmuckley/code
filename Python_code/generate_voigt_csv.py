...
filename = r'exp_data\voigt_'+str(step_num)+'.csv'
...
solutions = np.empty((len(combos), 2))
...
for row_i, row in enumerate(combos):
    ...
    solutions[row_i] = [deltaf, deltad]

# store results in a dataframe
df = pd.DataFrame(data=combos, columns=['mu', 'eta', 'rho', 'h'])
df['df'] = solutions[:, 0]
df['dd'] = solutions[:, 1]
