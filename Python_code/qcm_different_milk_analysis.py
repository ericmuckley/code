...
datafolder = r'C:\Users\a6q\exp_data\2019-08-19_milk_w_acetic_acid_qcm\*'

#%%

# get all files in folder with a certain string in the name
filelist = [f for f in glob.glob(datafolder) if '_spectra' in f]
# dictionary to hold all data
dic = {}

new_headers = []

# loop over each file in the folder and read it
for f in filelist:
    ...
    # read file
    df = pd.read_csv(f)
    # remove reactance columns
    df = df[[col for col in df.columns if 'xs' not in col]]
    # remove columns from overnight measurements
    df.drop(df.columns[16:-16], axis=1, inplace=True)
    
    # simplify headers on each dataframe
    new_headers = ['f', 'rs']*int(len(list(df))/2)
