# -*- coding: utf-8 -*-
"""
Created on Mon Jul 23 14:28:37 2018
...
index1, index2, skip_n = 1250, 2200, 1#6,-1, 1    
#225, 459, 1
...
for i in range(len(good_files)): 
    ...
    if band_col == 2:        
        ...
        #plot raw data
        ax_spec.scatter(freq0, G0, label='exp. data', s=15, alpha=0.5, c='k')    
        ...
        if bvd_fit == True: 
            ...
            ax_spec.scatter(freq0[peak_inds], peak_vals,
                    label='local max.', s=80, c='b', marker='+')
            ...
            ax_spec.plot(freq0, G_fit, label='BvD fit', c='r', lw=1)
            ...
            #fit peak position trajectory to exponential curve to predict it
            rh_future = np.array(rh_list[int(i/band_num):])
            rh_future=np.append(rh_future, np.linspace(98, 160, 31))
            
            if int(i/band_num) > 2:
                peak_ind_guess = [np.subtract(peak_ind_mat[0], peak_ind_mat[-1]),
                                  1,
                                  np.min(peak_ind_mat)]
                
                peak_val_guess = [np.subtract(peak_val_mat[0], peak_val_mat[-1]),
                                  1,
                                  np.min(peak_val_mat)]
            
            
            
                #predict future resonant frequency
                f_ind_params, _ = curve_fit(single_exp,
                                                rh_list[:int(i/band_num)],
                                                peak_ind_mat[:int(i/band_num)])#p0=peak_ind_guess)

                f_ind_fit = single_exp(rh_future, *f_ind_params)
                
                #predict future conductance max
                f_val_params, _ = curve_fit(single_exp,
                                                rh_list[:int(i/band_num)],
                                                peak_val_mat[:int(i/band_num)],
                                                p0=peak_val_guess)
                f_val_fit = single_exp(rh_future, *f_val_params)
                
                
                ax_spec.scatter(f_ind_fit, f_val_fit, c='g',
                            marker='+', s=5)
                ax_spec.plot(f_ind_fit, f_val_fit, c='g', linewidth=1)
                ax_spec.scatter(f_ind_fit[-1], f_val_fit[-1], c='g',
                            marker='+', s=80, label='prediction')
            ...
            for jjj in range(0, len(R_mat[0])):
                ...
                #plot R
                ax_R.scatter(rh_past, R_list, s=10)
                ax_R.plot(rh_past, R_list)
                #plot L
                ax_L.scatter(rh_past, L_list, s=10)
                ax_L.plot(rh_past, L_list)
                #plot C
                ax_C.scatter(rh_past, C_list, s=10)
                ax_C.plot(rh_past, C_list)
                #plot D
                ax_D.scatter(rh_past, D_list, s=10)
                ax_D.plot(rh_past, D_list)
                #plot f
                ax_f.scatter(rh_past, f_list, s=10)
                ax_f.plot(rh_past, f_list)
        
        #set up plots--------------------------------------------------------
        ax_spec.set_xlim(24.94, 24.95)
        ax_spec.set_ylim(-0.05, 4)
        ax_f.set_xlim(0,100)
        ax_D.set_xlim(0,100)
        ax_R.set_xlim(0,100)
        ax_L.set_xlim(0,100)
        ...
        ax_spec.legend(loc='upper left', ncol=1, fontsize=12)
        ax_spec.text(24.94175, 2.5, format(rh0)+' % RH',
