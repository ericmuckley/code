...
rho_f = 500 #film density in kg/m^3
...
#eta_range = np.linspace(0, 3e-4, step_num).astype(float)

exp_data = pd.read_table('mxene_3hr_df_dd_mags.txt')


'''
# Use this for testing simulated data:
df_exp_list = [-1, -5, -10, -20, -100, -1000, -2000, -10000]
dd_exp_list = [1, 1.5, 3, 5, 7, 15, 25, 50]
exp_data = pd.DataFrame(np.array([df_exp_list, dd_exp_list]).T,
                          columns=['df_exp', 'dd_exp'], dtype=np.float64
'''
...
for timestep in range(len(exp_data)):
    ...
    df_exp = -1e3*exp_data['df9'][timestep]
    dd_exp = 1e-6*exp_data['dd9'][timestep]
    ...
    #format plot
    ax.set_xlim([min(mu_range),max(mu_range)])
    ax.set_ylim([min(eta_range),max(eta_range)])
    ax.set_yscale('log')
    ax.set_xscale('log')  
    ...
    #save_pic_filename = 'voigt_intersection_frames\\'+format(
