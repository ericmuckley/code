...
def remove_outliers3(vec):
    ...
    for i in range(1, len(vec) - 1):
        #difference between point i's two adjacent points
        adjacent_diff = np.abs(vec[i+1]-vec[i-1])
    
        #differences between point i and its adjacent points
        diff1 = np.abs(vec[i]-vec[i-1])
        diff2 = np.abs(vec[i]-vec[i+1])
        
        #average difference between point i and its adjacent points
        avg_diff = np.average([diff1, diff2])

        #is point i an outlier?
        if avg_diff > 3*adjacent_diff:
            vec2[i] = np.average([vec[i-1], vec[i+1]])
    return vec2
...
with open('C:\\Users\\a6q\\exp_data\\saved_dict_SLG.pkl', 'rb') as handle:
    dic_slg = pickle.load(handle)
...
for key in dic2:
    ...
    if np.any(dic2[key]):
        ...
        for i in range(1, len(dic2[key][0])):
            plt.plot(dic2[key][:,0], dic2[key][:,i],label=str(i))
